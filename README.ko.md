...
| `mulgae-review` | 정확한 변경 타깃 하나 또는 명명된 task나 에픽 완료 후보에 대해 보고 전용 단독 Mulgae 리뷰를 한 번 수행하고, 그 수용된 역할 보고서를 판정합니다. | `/aquarium:mulgae-review` Mulgae 고유의 `stage`, `workspace`, `dirty`, `diff`, 또는 `patch` 타깃과 선택적 task 또는 에픽 ID |
...
네 개의 설계 스킬은 Ouroboros를 경계가 정해진 말단 기능으로 구동하므로 `>=0.51.1`을 만족하는 안정 릴리스가 설치되어 있어야 합니다 — 업스트림은 v0.1.17에서 상한을 없앴습니다. `/aquarium:dev-setup-global`이 별도 승인 뒤에 이를 진단하고 구성하며, 플러그인 자체는 `claude plugin`으로 설치합니다. 이 스킬들은 문서만 구체화하며 구현하지 않습니다.
...
두 정적 리뷰 스킬은 하나의 계약 `references/review-contract.md`를 공유합니다. 이 계약이 소스 스코프, 동의, 정적 리뷰 한계, 결과 봉투를 소유하고, Review Brief와 `change` 대 `completion` 목적은 `references/review-intent-contract.md`가, 판정은 `references/finding-disposition.md`가 맡으며, 타깃 검사기와 저장소 상태 검사기가 `independent-review` 안에 함께 배포됩니다. `orca-review`는 호스트 서브에이전트가 아니라 Orca 앱을 구동하므로 `references/orca-supervision.md`도 함께 읽고, 활성 카탈로그에 별도로 설치된 `orca-cli` Claude Code 스킬과 실행 중인 Orca 앱이 필요합니다. Orca의 오케스트레이션 가이드는 Orca 실행 파일 자체를 통해 읽으며, 그 스킬이 없으면 계약을 근사하지 않고 멈춥니다. `independent-review`는 서브에이전트 기반 경로로 남아 Orca 워커를 시작하지 않고 Dolgorae 캡처도 쓰지 않으며 추가로 필요한 것이 없습니다. 업스트림 v0.1.16은 자신의 Dolgorae 기반 경로를 비활성화하고 이제 호출자를 명시적으로 선택된 Orca 또는 호스트 네이티브 서브에이전트 리뷰로 보내는데, 이는 이 에디션의 스킬이 v0.1.14부터 이미 그래왔던 방식이므로 여기서는 계속 활성화되어 있습니다. 두 백엔드 모두 불변 캡처를 갖지 않으므로 지원 스코프는 `staged`, `head`, `commit`, `range` 네 가지로 같고, 업스트림의 캡처 전용 스코프 `workspace`와 `dirty`는 지원되지 않습니다. `mulgae-review`는 세 번째 단독 진입점입니다. Mulgae 자체의 네이티브 타깃에 대해 같은 Review Brief를 만들고 `references/mulgae-review-contract.md` 아래에서 수용된 역할 보고서를 소비하며, 네이티브 생애주기는 별도로 설치되는 `/use-mulgae` 스킬이 소유합니다. v0.1.17부터는 task, 에픽, 콜드 검증 워크플로도 `references/review-routing-contract.md` 아래에서 완료 리뷰 경로 하나 — 기본값인 Mulgae, 명시적 리뷰어를 지정한 Orca, 새 호스트 네이티브 서브에이전트 리뷰, 또는 명시적 면제 — 를 선택하며, 증거는 경로 중립적이고 평가 횟수에는 한도가 있습니다. 업스트림은 호스트 네이티브 경로를 `native-codex`라고 부릅니다. 여기서는 산문이 네이티브 Claude Code라고 말하되 경로 ID와 Podway 절차 레이블은 업스트림의 바이트를 그대로 유지하고, 이 경로는 아래에서 설명하는 번들 리뷰어 서브에이전트를 디스패치하며, `independent-review`는 선택 가능한 경로가 아니라 단독 진입점으로 남습니다.
...
플러그인은 `additions/agents/`에서 온 서브에이전트 하나 `aquarium:independent-reviewer`를 함께 배포합니다. 여기에는 업스트림 대응물이 없습니다. Codex에는 플러그인 서브에이전트가 없어 업스트림 리뷰 스킬은 Orca에 기대고, Claude 오버라이드는 "편집 도구가 없는 서브에이전트 타입"을 요청하는 산문에 기댔습니다. 번들 에이전트는 실제로 중요한 두 요구사항을 설정으로 바꿉니다 — 편집 도구가 없는 `Read`, `Grep`, `Glob`, `Bash` 도구 허용목록과 `model: opus` — 그리고 스킬의 리뷰어 요구사항에서 도출한 리뷰 중심 시스템 프롬프트를 담습니다. 리뷰어는 `git diff --cached`와 `git show`를 읽어야 하므로 Bash는 허용목록에 남으며, 도구 허용목록 밖의 읽기 전용 규율은 오직 프롬프트로 이루어지고 숨겨지지 않고 명시됩니다 — Claude Code는 플러그인 서브에이전트의 `permissionMode`를 무시하므로 에이전트는 이를 선언하지 않습니다. 프롬프트는 세션 내 리뷰어가 도구로부터 물려받을 수 없는 경계 하나도 담습니다. 서브에이전트는 같은 워크트리에서 같은 운영체제 사용자로 실행되므로, 작업 트리 사본이 아니라 Git 객체로 타깃을 읽고 배제된 dirty 잔여물을 열지 않는 것은 샌드박스가 아니라 규칙입니다. 대가는 플러그인이 활성화된 모든 세션에 이 에이전트의 설명이 적재된다는 점이며, 그래서 설명은 좁고 부정형으로 쓰여 있습니다. 번들 에이전트가 없으면 스킬은 편집 도구가 없는 아무 서브에이전트 타입으로 대체합니다. v0.1.16은 Orca 변경 리뷰를 비례적으로 유지하며, 번들 에이전트도 그 규칙을 그대로 따릅니다. 정확한 diff와 해당 authority부터 시작해 그럴듯한 영향 경로를 따라서만 확장하고, 타깃이 도입하지 않은 기존 이슈는 그대로 두며, 스타일 선호·추측성 위험·검증 공백은 발견으로 치지 않고, `completion` 목적에는 기준별 평가를 돌려주며, `APPROVE`는 코디네이터가 저장소 상태 비교 뒤에 확정하는 권고적 결론으로 보고합니다. v0.1.17은 이 에이전트에 두 번째 디스패처를 주었습니다. 워크플로의 `native-codex` 경로는 편집도 중첩 에이전트도 없는 새 서브에이전트 하나를 요구하는데, 이 에이전트의 허용목록은 이미 편집·쓰기·에이전트 디스패치 도구를 없애고 프롬프트는 Bash를 통한 쓰기를 금지합니다. 그래서 `task-review`, `epic-handler`, `epic-validator`는 이 에이전트를 Review Brief 전체에 대해 한 번 디스패치하고, 에이전트를 쓸 수 없으면 다른 서브에이전트 타입으로 대체하지 않고 preflight 실패로 멈추며, 그 체크포인트를 단독 Independent Review 실행으로 보고하지 않습니다. 설명은 두 디스패처만 이름으로 밝히고, 프롬프트는 Independent Review에서는 배정된 렌즈 하나를, 체크포인트의 유일한 리뷰어로서는 체크포인트의 평가 종류로 한정된 모든 렌즈를 적용합니다. 이 에디션에서 위임되는 모든 역할 — 이 리뷰어들, `release-qa`의 워커, `epic-handler`의 선택적 추가 관점 서브에이전트, `upgrade` 스킬의 서브에이전트 — 의 기준 모델은 Opus이고, Fable은 사용자가 명시적으로 요청할 때만 씁니다. 명시적 모델이 없는 서브에이전트는 세션의 모델을 물려받고, Fable은 usage credit으로 과금될 수 있으며 비대화형 실행에서는 묻지 않고 과금하기 때문입니다.

### 에디션 업그레이드 스킬

플러그인은 `additions/skills/`에서 온 에디션 소유 스킬 하나 `upgrade`를 함께 배포합니다. 업스트림 대응물이 없는 이유는 이 스킬이 이 생성기 저장소 자체를 다루기 때문입니다. 서브모듈을 릴리스된 업스트림 태그에 고정하고, 호출한 대화가 결정·검토·검증하고 모든 단계 경계에서 진행 상황을 보고하는 동안 오케스트레이션된 서브에이전트 — 판단과 리뷰는 Opus, 기계적 수집은 Sonnet, Fable은 명시적 요청 시에만 — 로 모든 동기화 중단을 해소하며, 검토된 태그와 GitHub Release와 로컬 설치 갱신을 명시적 승인 뒤에 배포합니다. 이 저장소 밖에서는 실행을 거부하며, 함께 배포되는 `references/rederivation.md`가 오버라이드 재도출 방법과 코드에서 보이지 않는 채택 함정들을 기록합니다. 다른 addition과 마찬가지로 최종 형태로 저작됩니다. 게이팅 플래그는 스스로 선언하고, 생성은 그 플래그가 없는 스킬 addition을 거부하며, `tests/validate.rb`는 사이드카 없는 스킬이 기록된 addition이면서 게이트되어 있음을 단언합니다.
...
`sync.py`는 업스트림 플러그인을 복사하고, 이름으로 제외된 파일과 스킬을 버리고, 문자열 치환을 적용하고, 오버라이드를 적용하고, 업스트림 사이드카에서 호출 게이팅을 도출한 뒤 사이드카를 버리고, 마지막으로 호스트 전용 파일 — 번들 리뷰어 서브에이전트, 에디션 소유 `upgrade` 스킬, 그리고 `independent-review`가 실행하는 저장소 상태 검사기 — 를 더하며, 그 스킬이 게이팅을 스스로 선언하지 않으면 생성을 거부합니다. 빈 서브모듈에 대해서는 실행을 거부하고, 변환이 다루지 않는 플러그인 루트 디렉터리나 파일이 업스트림에 생기면 실행을 거부하며, 호스트 고유 텍스트가 살아남으면 실패하고, 출하되는 텍스트를 하나도 고치지 못한 치환 규칙이 있으면 실패하고, 생성된 스크립트가 실행될 수 없으면 실패합니다.

여섯 개의 파일이 의미적으로 갈라져 치환이 아니라 오버라이드로 유지됩니다.

| 오버라이드 | 이유 |
|---|---|
| `references/review-contract.md` | v0.1.14는 공유 계약을 업스트림의 새 백엔드 분리에 맞춰 다시 쓰면서 independent review를 Dolgorae 불변 캡처와 신규 Codex Reviewer에 못박았습니다. 두 축 모두 여기서는 참이 아니고, 그 차이가 한 절이 아니라 스코프 표·백엔드 소유권·정산·결과 봉투 전체를 관통합니다. 이 오버라이드는 업스트림의 스코프 의미, 동의, 정적 한계, 판정 규칙을 유지하되 캡처 전용 스코프 두 가지를 Orca Review와 같은 이유로 미지원으로 표시하고, 캡처 대신 저장소 상태 기준선을 두어 타깃이 불변이었음이 아니라 리뷰 중 아무도 저장소를 변경하지 않았음을 증명합니다. v0.1.16은 Review Brief, `change` 대 `completion` 목적, 비례적 변경 검사, Orca의 운영 편차 정책을 담습니다. 오버라이드는 이 네 가지를 모두 채택하고 비례 규칙을 두 백엔드 모두에 적용하며, 업스트림이 이제 비활성화된 경로로 기록한 지점에서도 서브에이전트 백엔드를 유지합니다. v0.1.17은 task, 에픽, 또는 검증 핸들러가 승인된 봉투 안에서 리뷰를 위임할 수 있게 합니다. 오버라이드는 그 동의 규칙과 라우팅 계약 링크를 이식하되, 여기서는 어떤 핸들러도 Independent Review를 호출하지 않으므로 Orca로 한정합니다. |
| `skills/independent-review/SKILL.md` | 업스트림의 Dolgorae-그리고-Codex 백엔드를 호스트 자체 메커니즘으로 파견되는 새 읽기 전용 서브에이전트로 대체하며 번들 `aquarium:independent-reviewer`를 우선합니다. 리뷰어는 코디네이터와 같은 프로바이더에서 실행되므로, 이 스킬은 독립 프로바이더가 아니라 새 컨텍스트를 주장하고, 깊이는 Opus에 넓이는 Sonnet에 두며, 여러 리뷰어에게 서로 다른 렌즈를 주어 커버리지를 얻습니다. 각 렌즈의 모델은 명시적으로 전달하며, 렌즈에 Fable을 요청하는 것은 사용자가 명시적으로 요청할 때뿐입니다. 공유 계약은 프로세스 밖 리뷰어 하나를 전제로 쓰였으므로, 이 오버라이드는 계약의 단수 리뷰어·판정·백엔드 생애주기 필드를 파견된 서브에이전트당 한 행으로 매핑하기도 합니다. 업스트림의 v0.1.16 파일은 비활성화된 Dolgorae 경로를 위한 거부 스텁이므로, 오버라이드는 이제 스킬 전체입니다. Review Brief와 목적을 모든 리뷰어 명세에 담고, 업스트림의 비례 리뷰와 조치 가능한 발견 규칙을 리뷰어들에게 적용하며, 저장소 상태 비교 뒤에 코디네이터가 최종 기술 판정을 내리게 합니다. |
| `skills/dev-setup-global/SKILL.md` | v0.1.15는 사용자 전역 설치를 `dev-setup`에서 분리했고, 이 에디션의 전역 분기도 그와 함께 옮겨 갔습니다. 짝 스킬과 서드파티 스킬은 설정된 Claude Code 스킬 루트 아래에 설치되고, im-not-ai는 자기 Claude 페이로드를 실체화하며, Ouroboros 구성요소는 이 호스트가 가진 설치 하나로 축소됩니다. Ouroboros는 home별 규칙·스킬·MCP 등록이 아니라 플러그인으로 도달하므로, 전역 설정은 CLI 버전과 플러그인 스코프 등록을 보고하고 설치와 업그레이드는 `claude plugin`에 맡깁니다. 번들 `aquarium-dev` 채널은 여기서 배포되지 않으므로 제안되지도 않습니다. v0.1.16은 플러그인 설치와 업데이트를 호스트 자신의 플러그인 관리로 보내며, 여기서는 그것이 `claude plugin`입니다. v0.1.17은 Dolgorae 릴리스 검증기를 제거했으므로 3단계는 더 이상 그 플래그를 넘기지 않으며, 중복 규칙은 선택된 타깃 옆에 함께 적재될 사본을 중심으로 고쳐 쓰였습니다. production-status 런타임에 관한 문장들은 그 런타임이 제외되어 있으므로 이식되지 않고, 다른 호스트의 스킬 home에서 스킬 타깃을 해석하거나 im-not-ai를 활성 home 아래에 두는 새 문장들도 이식되지 않습니다. 이 에디션의 설치는 언제나 구성된 Claude Code 스킬 루트를 대상으로 하기 때문입니다. home별 Ouroboros 문단은 계속 빠진 채로 남습니다. |
...
| `references/tool-catalog.md` | Mulgae와 Gaori MCP를 `claude mcp add -s user`로 등록하고, `.mcp.json`을 명시적 프로젝트 재정의로 두며, 사용자·프로젝트·유효 뷰를 `.codex/config.toml`과 타입이 있는 `codex mcp get --json` 출력이 아니라 설정 파일에서 검증합니다. v0.1.15는 이를 `dev-setup` 밖의 공유 `references/`로 옮겼고, 제외된 채널과 함께 `aquarium-dev` 절을 버렸으며, 업스트림의 home별 Ouroboros 장치를 이 호스트의 플러그인 통합으로 대체했습니다. v0.1.16은 Sanho, Mulgae, Gaori, Sorage의 최저 버전을 올리고 부재 증명으로 `codex mcp list --json` 인벤토리를 더했습니다. 최저 버전은 이식되지만 인벤토리는 이식되지 않는데, `claude mcp list`가 승인된 서버마다 헬스체크를 수행해 서버를 시작시키기 때문입니다. 제외된 개발 채널의 생산자 매니페스트에 관한 문장은 그 채널과 함께 버립니다. v0.1.17은 Dolgorae 검증기를 제거했고, Mulgae를 ZCode, Grok, Codex 기본 포트폴리오와 함께 v0.1.23 이상으로 옮겼으며, Humanizer, im-not-ai, Ouroboros를 각자의 최저 버전 이상인 어떤 안정 릴리스에도 열어 두었고, `use-podway`를 Podway v0.2.11 릴리스 커밋에 고정했습니다. 이 사실들은 이식되지만 그 production-status 절과 다른 호스트의 스킬 home을 고르는 문장들은 이식되지 않으며, 새 스킬 위치 규칙은 다른 호스트의 릴리스가 어떤 기본값을 문서화하든 설정된 Claude Code 스킬 루트 아래에 설치합니다. |
| `references/review-intent-contract.md` | v0.1.16에 공유 Review Brief와 `change` 대 `completion` 계약으로 새로 추가되었으며, 업스트림의 비활성화된 경로를 위해 쓰인 절 두 개를 담고 있습니다 — 명시적으로 미리 선택된 Orca 또는 "네이티브 Codex" 서브에이전트 리뷰로 `independent-review` 호출자를 보내는 라우팅 매트릭스, 그리고 그 네이티브 경로에 대한 설명입니다. 여기서는 `independent-review`가 곧 네이티브 서브에이전트 경로이고 계속 활성화되어 있으므로, 오버라이드는 Review Brief·출처·후보 경계·평가 절은 그대로 두고 그 두 절과 계약의 첫 문장만 이 에디션이 가진 백엔드에 맞춰 다시 씁니다. v0.1.17은 평가 종류 — `work-unit` 또는 `remediation-confirmation` — 를 더하고 새 워크플로 라우팅 계약을 링크합니다. 둘 다 그대로 이식되며, 라우팅 절은 이제 Independent Review가 선택 가능한 워크플로 경로가 아니라고 말합니다. 내장된 워크플로는 `native-codex`를 통해 네이티브 리뷰에 도달하기 때문입니다. 네이티브 절은 번들 리뷰어의 두 디스패처 — Independent Review의 렌즈와 워크플로의 `native-codex` 체크포인트 — 를 모두 밝히고, 두 실행이 서로로 보고되지 않게 합니다. |

`skills/dev-setup/SKILL.md`은 v0.1.15가 사용자 전역 설치를 `dev-setup-global`로 옮기기 전까지 오버라이드 하나였고, 그 자리는 `skills/dev-setup-global/SKILL.md`이 대신했습니다. 지금도 갈라지는 것은 문자열 구간 세 곳이고 그중 둘은 금지 니들을 담고 있어 오버라이드가 남든 남지 않든 규칙이 필요했으므로, 오버라이드는 폐기되고 치환 세 개가 그 자리를 대신했습니다 — 횟수를 센 교체가 폐기된 오버라이드를 바이트 단위로 재현합니다. v0.1.17은 프로덕션 설정 원장을 이 파일과 `dev-setup-bundle`에 연결했고, 그 구간들 — 파일마다 세 곳 — 도 횟수를 센 치환으로 제거됩니다. 업스트림은 두 파일을 거의 매 릴리스마다 고치고, 오버라이드를 두면 그 파일들의 다른 구간을 맡는 규칙들이 죽기 때문입니다.

나머지는 전부 문자열 치환입니다. `$aquarium:` 시길은 `/aquarium:`이 되고, `$use-*` 스킬 시길은 `/use-*`가 되며, 유지보수 스킬 시길 `$create-podway-procedure`는 `/create-podway-procedure`가 되고, Ouroboros 시길 `$interview`, `$pm`, `$seed`, `$qa`는 Ouroboros가 사용자 스코프 스킬이 아니라 Claude Code 플러그인으로 설치되므로 `/ouroboros:*`가 되고, 별도 설치되는 `$deslop`은 `/deslop`이 되며, `request_user_input`은 `AskUserQuestion`이 되고 — task-close의 "가용할 때" 헤지는 이 호스트에서 그 도구가 항상 존재하므로 제거되며, Lora는 `--agent claude-code`로 설치되며, 워크플로우가 연동하는 `Codex goal`은 Claude Code 세션 goal — 사용자가 설정하거나 해제하고 평가 모델이 충족되었거나 불가능하다고 판단하면 끝내는 `/goal` 완료 조건이며, Fable 5.1과 Opus 5.5에서 기본으로 꺼져 있는 Claude Code의 todo 도구로는 대신할 수 없습니다 — 이 되고, 모델이 그 goal을 만들거나 완료하거나 막힘으로 표시하게 하던 문장은 그 goal 아래에서 일하고 보고하도록 다시 쓰이며, 검사 스크립트는 스킬을 Claude Code 루트 — `CLAUDE_CONFIG_DIR`와 `~/.claude/skills` — 에서만 해석하고 Codex 대신 이 호스트에 대해 Ouroboros를 진단하며, 사용자 스코프 스킬은 `~/.claude/skills`에 설치되고, `release-qa`의 호스트 중립적인 "available agent delegation surface"는 이 호스트 자신의 서브에이전트 메커니즘이 되며 독립 클러스터는 단일 메시지로 병렬 파견되고 모든 워커는 사용자가 Fable을 명시적으로 요청하지 않는 한 Opus로 실행되며, `epic-handler`의 선택적 추가 관점 서브에이전트도 같은 조건으로 Opus를 요청하고 — 도구 이름이 아니라 메커니즘을 대는 것은 Claude Code가 그 이름을 이미 한 번 바꿨기 때문이고 — `orca-review`의 캡처 전용 스코프 두 가지에 대한 거부는 그것이 비활성화되었다는 것이 아니라 independent review도 그 스코프들을 지원하지 않는다고 말하며, 공유 판정 계약은 independent review를 이 호스트 자신의 서브에이전트 경로로 명명하고 그것을 캡처하는 백엔드와 묶기를 멈추고, 업스트림의 호스트 네이티브 리뷰 경로는 `native-codex` ID를 유지한 채 네이티브 Claude Code로 읽히며 번들 리뷰어 서브에이전트를 디스패치하고, 라우팅 계약은 Independent Review가 비활성화된 진입점이 아니라 살아 있는 단독 진입점이라고 말하며, 프로덕션 설정 원장 문단들은 `dev-setup`과 `dev-setup-bundle`에서 빠지고, 쓰기 스킬 검사는 Humanizer와 im-not-ai를 공유 크로스 에이전트 루트와 Codex home이 아니라 Claude Code 스킬 루트에서 찾으며, v0.1.15가 짝 스킬과 서드파티 스킬을 위해 더한 존재만 확인하는 신뢰 표도 같은 루트를 통해 해석되고, 훅 명령은 Codex의 `${PLUGIN_ROOT}` 대신 `${CLAUDE_PLUGIN_ROOT}`를 해석합니다.
...
문자열 치환 규칙은 반대 방향으로도 똑같이 조용히 실패합니다. v0.1.10은 `Ouroboros package, Codex, and runtime components`에서 옥스퍼드 쉼표 하나를 없앴고 규칙은 아무 말 없이 매칭을 멈췄으며, 같은 릴리스에서 업스트림이 공유 헬퍼를 추출하고 매개변수를 더하면서 스크립트 규칙 세 개가 죽었습니다. v0.1.13에서도 업스트림이 Ouroboros transport 비교를 두 개의 런처 matcher로 추출하고 런타임 구성요소를 설치·미설치 경로로 나누면서 스크립트 규칙 세 개가 다시 죽었고, 새 앵커에 맞춰 재도출되었습니다. 이제 모든 규칙은 출하되는 텍스트를 고쳐야 합니다. 생성은 오버라이드 대상 밖의 매칭 횟수를 세고 아무것도 고치지 못한 규칙의 이름을 대며 실패하므로, 죽은 규칙은 썩는 대신 의도적으로 삭제되거나 재도출됩니다. 오직 오버라이드 대상 안에서만 나타나던 구문들도 같은 이유로 삭제되었으며, 오버라이드가 언젠가 폐기되면 금지 니들과 시길 스캔이 여전히 그 텍스트를 잡습니다. v0.1.16에서는 스크립트 규칙 여덟 개가 한꺼번에 죽었습니다. 여섯은 그 탐침 형태 변화가 옛 탐침을 읽던 모든 함수에 미쳤기 때문이고, 둘은 업스트림이 `inspect_im_not_ai`와 신뢰 표에 공유 크로스 에이전트 스킬 루트를 하드코딩했기 때문입니다. Ouroboros 등록 규칙 두 개는 함수 전체 규칙 하나가 되었고 나머지는 다시 앵커링되었습니다. v0.1.17은 스크립트 규칙 다섯 개와 마크다운 규칙 세 개를 죽였습니다. 업스트림이 쓰기 스킬 루트 선택을 헬퍼 두 개로 옮기고, 옛 교체 코드가 부르던 버전 상수와 키워드의 이름을 바꾸고, `inspect_im_not_ai`와 신뢰 표를 그 헬퍼들 중심으로 다시 쓰고, Ouroboros 상한을 없애고, 마크다운 규칙들이 앵커로 삼던 판정 문장과 Ouroboros 통합 문장을 다시 썼기 때문입니다. 스크립트 규칙들은 옛 교체 코드가 아니라 새 함수들로부터 재도출되었고, 두 헬퍼는 마지막 호출자와 함께 삭제되며, 검증기는 그것들에 대한 참조가 하나라도 남아 있으면 거부합니다.
...
v0.1.17은 그 디렉터리 아래에 두 번째 도구를 두었습니다. 바로 `aquarium-status` 프로덕션 설정 원장 런타임으로, 새 `status` 스킬이 이를 보고하고 업스트림은 이를 `dev-setup`, `dev-setup-bundle`, 전역 검사기에 연결했습니다 — `dev-setup`은 런타임이 원장의 선행 행을 읽을 수 있을 때까지 저장소 변경을 시작하지 않도록 되어 있었습니다. 이 런타임은 채널과 함께 제외된 채로 남고, `status`는 통째로 제외됩니다. 이 스킬이 밟는 모든 단계가 그 런타임을 실행하고, 이 에디션이 배포하지 않는 `aquarium-dev` 채널의 등록 상태를 보고하며, 그 기록 봉투는 플러그인 밖에 있는 업스트림의 저장소 수준 `docs/specs/production-status.md`에만 명세되어 있고, 이를 유지하면 Apple Silicon 전용 Python 런타임과 `~/.aquarium` 아래에서 다른 호스트의 에디션과 공유되는 원장이 모든 저장소 설정의 전제 조건이 되기 때문입니다. 횟수를 센 치환이 두 설정 스킬에서 원장 문단을 제거하고, 스크립트 규칙이 전역 검사기의 해당 구성요소를 제거합니다. 그러지 않았다면 그 구성요소의 로더가 스코프를 지정하지 않은 모든 검사를 실패시켰을 것입니다. 스킬은 파일 단위로 제외할 수 없습니다 — 생성은 `SKILL.md`가 없는 스킬 디렉터리를 거부하고, `SKILL.md`에 대한 basename 검사는 모든 스킬에 매칭될 것입니다 — 그래서 `EXCLUDED_SKILLS`가 어떤 텍스트 규칙이 실행되기도 전에 그 스킬을 통째로 제거하고, 그 스킬이 업스트림에 여전히 존재할 것을 요구하며, 그 스킬을 가리키는 생성된 참조를 모두 거부합니다.

새 도구는 디렉터리 제외가 업스트림이 그 아래에 더하는 것을 무엇이든 감춘다는 점도 보여 주었습니다. `tools/aquarium-status/`가 제외된 채널 옆에 나타났을 때 아무것도 중단되지 않았습니다. 이제 생성과 검증기는 제외된 플러그인 루트 디렉터리의 하위 항목을 가리키는 생성된 경로를 모두 거부하며, `aquarium-status`와 `production-status`는 금지 니들입니다.
...
그 밖에 `Codex`라는 이름은 생성된 텍스트에서 금지됩니다. 한 파일이 면제됩니다. `tool-catalog.md`는 Codex CLI를 Mulgae 리뷰 프로바이더로 명명하며 — 프로바이더 옵트인, 필요한 CLI 버전, 네이티브 로그인, 여러 자격 증명 신원, 실행 파일, 그리고 v0.1.17부터는 Mulgae 기본 포트폴리오의 세 프로바이더 중 하나 — 이는 여기서도 참입니다. 면제는 그것이 판단된 업스트림 다이제스트를 기록하므로 그 파일이 바뀌면 동기화가 멈춥니다. v0.1.11은 `orca-review/references/provider-contracts.md`를 명명된 프로바이더 중심으로 다시 써서 `Codex` 언급을 남기지 않았으므로 그 면제는 회전이 아니라 폐기되었고, v0.1.15는 업스트림이 그 스킬을 삭제하면서 `aquarium-dev` 면제 두 개를 폐기했습니다. 회전할 때마다 다이제스트가 움직이기 전에 오버라이드가 출하하는 모든 언급을 다시 읽습니다.

Podway 절차 세 개는 다른 경우입니다. v0.1.17은 그 선택지와 기준에서 호스트 네이티브 리뷰 경로에 "native Codex"라는 레이블을 붙이고, Podway는 설치된 절차 사본이 번들된 바이트와 일치할 것을 요구하므로, 이 절차들은 다시 쓸 수도 없고 면제해도 소용이 없습니다 — 검토된 면제는 릴리스마다 수천 줄씩 바뀌는 파일들 속의 모든 언급을 받아들이게 될 것입니다. 구문 범위로 한정된 용인이 Podway 절차 자산 안에서 그 경로 레이블만 가립니다. 그 안의 다른 금지 텍스트는 여전히 실패를 일으키고, 그 구문이 더 이상 등장하지 않거나 그 범위가 규칙이 다시 쓰는 파일에 닿으면 생성이 멈추며, 검증기는 같은 표를 유지하며 이를 집합으로 비교하고, 그 파일들이 업스트림과 바이트 단위로 동일하게 남아 있음을 단언합니다.

Mulgae와 Gaori MCP 탐침도 같은 이유로 재조준되었습니다. 업스트림은 이를 `codex mcp get --json`으로 읽고 로컬 뷰를 위해 `CODEX_HOME`을 `.codex/`로 향하게 하므로, 이 호스트에서는 검사기가 자기 카탈로그가 사용자에게 만들라고 지시한 등록을 결코 볼 수 없었고 `--require-mulgae-mcp`는 그 도구를 영원히 degraded로 보고했습니다. 생성된 검사기는 Claude Code의 세 가지 뷰를 설정에서만 읽습니다 — `CLAUDE_CONFIG_DIR`를 존중하며 `.claude.json`에서 사용자 스코프 항목과 비공개 프로젝트별 항목을, `.mcp.json`에서 공유 항목과 그 승인 상태를 읽고 — 문서화된 우선순위에서 유효 뷰를 도출합니다. `claude mcp get`은 의도적으로 결코 호출하지 않습니다. 그것은 승인된 서버를 헬스체크하므로 서버를 시작시키는데, 설정 작업은 서버를 시작시켜서는 안 되기 때문입니다. 아직 아무도 승인하지 않은 `.mcp.json` 서버는 degraded가 아니라 `registration_pending_approval`을 동반한 `unverifiable`입니다. 앵커가 업스트림이 가진 가장 안정적인 텍스트가 되도록 함수 두 개를 통째로 교체하며, `tests/test_claude_mcp_inspection.py`가 비공개 `CLAUDE_CONFIG_DIR` 아래의 설정 픽스처에 대해 출하되는 바이트를 실행합니다. v0.1.16은 neutral-cwd 사용자 스코프 탐침을 전역 검사기 밖으로 옮겨 프로젝트 검사기 안의 새 `inspect_global_mcp_scope`로 이동시켰고, 규칙도 그와 함께 옮겨갔습니다. 그 함수는 이제 설정 읽기의 사용자 스코프 뷰를 반환하며, 전역 검사기의 한 줄짜리 위임은 업스트림이 작성한 그대로 출하됩니다.
