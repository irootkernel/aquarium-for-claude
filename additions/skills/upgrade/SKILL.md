...
- Tell the user the phase plan when the upgrade begins, then report at every phase boundary and approval gate what finished, the evidence it produced, and the next gate. Do not depend on a todo or task-list tool: Claude Code does not give one to every model, and the user's approvals are the checkpoints that must stay visible.
...
- Set the model on every dispatch rather than letting it inherit the session's. Opus does all judgment and review — the upstream delta analysis, semantically delicate re-derivations, and the pre-commit review — because it is this edition's baseline for delegated work; Sonnet does mechanical collection and the counted swaps whose exact text you already fixed, where exact counts check its output. Request Fable only when the user explicitly asks for it: it can bill usage credits, and a non-interactive run bills them without asking.
...
Run `python3 scripts/sync.py` repeatedly. Every abort names its own fix and stops generation rather than producing partial output; resolve one class at a time and re-run until generation succeeds. Read `references/rederivation.md` before resolving the first abort — it records the counted-swap method and this edition's known traps — and record there every new trap this run finds that the code cannot show, in the adoption commit, so the next run starts from it. Work the classes in this order, because earlier states gate later checks:
