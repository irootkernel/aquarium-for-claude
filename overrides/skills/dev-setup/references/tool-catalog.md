...
Treat MCP as an optional, separately approved project-local component. For a trusted project, merge this entry into the repository's `.mcp.json` while preserving unrelated configuration:
...
The plugin declares its server as `plugin:ouroboros:ouroboros` and launches it with `uvx --from ouroboros-ai[mcp] ouroboros mcp serve`, so `uv` must already be present. That resolution is independent of the pinned `uv tool install` above and can select a version outside the supported range, so report the CLI version and the server separately and never treat one as evidence for the other.

Diagnose four independent components with the v6 inspector's explicit `--include-ouroboros` flag: `ooo --version`, the plugin's registration, `ooo mcp doctor --json`, and the effective MCP registration. These probes are local and read-only: they do not contact a provider, initiate authentication, or make a network request, though the MCP doctor may inspect bounded local authentication-readiness metadata without exposing credential material. A healthy CLI does not prove that the plugin is installed, that the MCP runtime is ready, or that the server is reachable.
