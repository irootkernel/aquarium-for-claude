...
Ouroboros ships its Claude Code integration as a plugin rather than as user-scoped skills, so `/ouroboros:interview`, `/ouroboros:pm`, `/ouroboros:seed`, `/ouroboros:qa`, and the MCP server all arrive together with it. Install it from its own marketplace as a separate approved mutation:
...
That resolution is independent of the pinned `uv tool install` above and can select a version outside the supported range, so report the CLI version and the server separately and never treat one as evidence for the other.

Diagnose four independent components with the v14 inspector's explicit `--include-ouroboros` flag: the CLI through `ooo --version`, the plugin integration and the effective MCP registration through `claude mcp get plugin:ouroboros:ouroboros` with a bare `ouroboros` fallback, plus runtime configuration derived from the plugin-scoped launcher. Run `ooo mcp doctor --json` only for a hand-registered entry that directly launches the selected `ooo` executable, because it inspects that package environment. These probes are local and read-only: they do not contact a provider, initiate authentication, or make a network request, though the direct MCP doctor may inspect bounded local authentication-readiness metadata without exposing credential material. A healthy CLI does not prove that the plugin is installed, that the MCP runtime is configured, or that the server is reachable.
...
Package installation or upgrade, plugin installation, and the selected configuration action are separate persistent mutations with separate approvals. For full configuration, run only `ooo setup --runtime claude --non-interactive`: it installs and prunes the packaged rules and upstream Ouroboros skills for the selected runtime and writes the runtime and LLM backend into `~/.ouroboros/config.yaml`, and it deliberately registers no MCP server, because that profile cannot share the isolated server process the plugin launches; omit `--mcp-mode`, which selects another host's MCP configuration mode. Do not run `ooo setup refresh` before full setup. Offer refresh only as a separately approved rules-and-skills repair alternative when full setup is not selected. The plugin is therefore the only supported source of the server: never hand-register a plain `ouroboros` entry to compensate. Re-read exact targets before each approved mutation and stop if they changed.

No setup action authorizes a provider call, authentication, repository-source transmission, `auto`, `run`, `ralph`, `evolve`, Seed creation, or an Aquarium design workflow. Verify only version, plugin registration, effective registration, and, when safely observable in the active host, live MCP tool exposure. A Claude Code restart may be required after skill or MCP changes.
