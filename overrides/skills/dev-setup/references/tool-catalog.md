...
Diagnose four independent components with the v10 inspector's explicit `--include-ouroboros` flag: the CLI through `ooo --version`, the plugin integration and the effective MCP registration through `claude mcp get plugin:ouroboros:ouroboros` with a bare `ouroboros` fallback, plus runtime configuration derived from the plugin-scoped launcher. Run `ooo mcp doctor --json` only for a hand-registered entry that directly launches the selected `ooo` executable, because it inspects that package environment. These probes are local and read-only: they do not contact a provider, initiate authentication, or make a network request, though the direct MCP doctor may inspect bounded local authentication-readiness metadata without exposing credential material. A healthy CLI does not prove that the plugin is installed, that the MCP runtime is configured, or that the server is reachable.
