...
For `Diagnose only`, report the presence and coverage of the required structure, missing commit-message authority, duplicated or conflicting guidance, CLAUDE.md delegation state including whether it delegates through a real `@AGENTS.md` import or in prose only, and the local evidence available for project indexing. Do not draft or mutate files.
