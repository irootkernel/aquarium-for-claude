...
6. Immediately before dispatch and after any approved staging, resolve `../orca-review/scripts/inspect_repository_state.py` from this skill directory — the repository-state helper shipped beside `orca-review` — and run it with `--repository <exact-git-root> --snapshot`. Bind its complete JSON result as the coordinator-owned no-mutation baseline; a helper that is missing or fails is a reported gap, not a reason to improvise a substitute snapshot.
...
Return the complete shared result envelope, reading its single reviewer identity, verdict, and backend lifecycle status as one row per dispatched reviewer — subagent type, assigned lens, requested model, its own verdict, and its dispatch status — over one adjudicated result for the whole review, and identify this host's subagent mechanism as the backend. Return `APPROVE` only when every dispatched reviewer reported, each examined the intended target and authority, no actionable finding remains, and the repository-state comparison reports no drift. Wrong scope, repository-state drift, missing output, or a failed or unreported dispatch prevents a clean verdict.
