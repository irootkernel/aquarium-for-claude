...
Use this contract whenever Aquarium consumes findings from Mulgae Review, Orca Review, Independent Review, an explicitly selected native Claude Code review subagent, or a workflow review waiver. Read [review-intent-contract.md](review-intent-contract.md) when the review carries change or completion intent and [review-routing-contract.md](review-routing-contract.md) when an embedded workflow selects or changes routes. A reviewer or coordinator finding is an advisory hypothesis. The coordinator checks it against the exact reviewed target, repository authority, production callers, persistence and concurrency boundaries, and existing tests before deciding what it means or what happens next.
