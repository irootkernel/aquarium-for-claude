...
Unless the workflow is already opted out, verify that Podway is ready for Aquarium use before requesting plan or execution-envelope approval. Readiness requires the supported stable `v0.2.11` through `v0.2.x` CLI and matching daemon on native Apple Silicon macOS, reachable healthy workspace state, `.podway/config.yaml`, `.podway/.gitignore`, and all five managed Procedure paths as tracked regular non-symlink files with the expected filename and Procedure ID that pass `podway procedure check --warnings-as-errors`. Each file must also satisfy the objective handler contract in [Procedure Node Contracts](procedure-node-contracts.md). Canonical bytes are compatible by definition; a same-ID customization remains preserved but cannot enter automatic managed execution when its required nodes, items, selected evidence, or routes are missing or cannot be established. Use bounded `podway --json daemon wait-ready --timeout 120s`; healthy requires a `podway.daemon-status-result/v3` payload with `mode=prod`, ready state and stage, null or bounded activity counts, and completed worktree recovery. A nonzero failed count may represent quarantined completed recovery and does not independently degrade readiness. An exact v0.2.5 Procedure workaround is migration evidence, not readiness, and must be replaced with canonical bytes before a managed workflow starts:
...
Neither a setup repair nor an Aquarium workflow may invoke `workspace remove`. That command deletes the selected worktree's complete `.podway` tree and belongs only to the pinned `/use-podway` flow after an explicit exact-worktree request, complete deletion disclosure, fresh root and workspace-UUID validation, and a `podway.workspace-removal-result/v1` success. It preserves the Git worktree and grants no daemon uninstall, log purge, Git cleanup, commit, or publication authority.

Neither a setup repair nor an Aquarium workflow may move an initialized workspace between runtime modes. Only an explicit request naming the exact worktree and target mode may hand off `workspace mode plan` and `workspace mode apply` to the pinned `/use-podway` flow. Before apply, disclose that the operation rewrites tracked `.podway/config.yaml` and deletes every current and archived session, attempt, queue entry, receipt, and job-history row. The plan remains read-only. Retain its exact token as sensitive ephemeral state, revalidate the source workspace UUID and both runtime modes, obtain separate apply approval, and derive apply or recovery only from current native output. Re-observe the target mode after convergence. Any Git action on the changed configuration remains separately authorized.
...
- Claude Code todo lists and Podway goals have separate creation rules and lifecycles. Create a Claude Code todo list only when explicitly requested under its current tool contract; a Podway goal does not authorize one. When both are authorized, keep their scope consistent with the user objective and roadmap without copying the complete criteria or status history. A bounded Podway session may cover only part of the Claude Code todo list.
...
## Resume and Coordinate Goals

Follow the pinned skill's context recovery guidance when resuming an authorized Podway workflow. Read its `references/goal.md` when a Claude Code todo list accompanies that workflow. Context restoration or automatic continuation of the same workflow retains its established authorization and boundaries; it does not repeat activation or the entry archive advisory.

Before the next Podway action after restoration, observe the authorized owning worktree and compare its workspace UUID and session ID with the recorded workflow identities. Stop session mutations on an identity mismatch unless existing authorization covers it. Within the same authorized session, use the current attempt and revisions without renewed activation. Recover an uncertain submitted mutation with its original canonical request and idempotency key before issuing another mutation.

Keep notes focused on the objective, scope, decisions, unresolved dependencies, evidence locations, and identity or request-recovery information. Historical observations are lookup hints, not current mutation templates. Context loss alone does not invalidate external checks whose target and relevant inputs still match and whose reuse the Procedure permits; preserve required readback and freshness checks.

A terminal session, null guidance, or an `achieved` assessment does not independently prove the host objective complete. Verify the actual requirements and external results, and continue any remaining authorized work. A Podway blocker does not automatically block a Claude Code todo list: follow that list's current tool contract, including any recurrence threshold, and continue useful independent work. Never change Podway lifecycle state to synchronize goal statuses.
...
- **Finish or cancel while preserving history:** Follow `/use-podway` for the exact current lifecycle action and its required approval. Do not claim that Aquarium must find or invoke the skill that previously used the session.
- **Delete:** Follow `/use-podway`'s current-session discard flow, including observation, fenced dry-run, disclosure of history loss, and separate explicit deletion authorization.
...
Never cancel, delete, reset, replace, or invent a terminal disposition merely to free the slot. A healthy existing session is a lifecycle choice for the user, not degraded Aquarium readiness and not a reason to route to `/aquarium:dev-setup`.

Never cancel, reset, force-replace, reopen, or reinterpret a blocking lifecycle conflict automatically. When deletion itself is the user's intent and no approved successor will replace the session, provide an exact `/use-podway` current-session discard handoff naming the repository and observed session ID; that handoff grants no cleanup authority by itself. Reset is deletion, not preparation for a successor workflow.
...
After a fresh supported disposition, re-observe and use only the eligible reset or `session.start_replace` template exposed for that exact revision. When an approved successor workflow needs the worktree slot, execute the current plain `start` template so Podway atomically archives the disposed predecessor and creates the prepared successor; then re-observe and `begin` it. Prepared and running sessions follow the current start policy and are preserved unless an explicit `--on-existing delete` choice is authorized. Inspect inactive history through `archive list` (`podway.session-archive-list-result/v2`) and `archive show`. The list reports an exact count, a newest-first window, and the oldest summary. Purge only through separately authorized `archive purge`; never auto-purge or evict history. A standalone task, validation, design, war-room, or final epic session is not reset merely because it is terminal.
...
Read [Procedure Node Contracts](procedure-node-contracts.md) for the concise
capability, input, prohibited-effect, return-evidence, rework, and resume contract
used by action instructions and handler compatibility inspection.

- `aquarium-task-v2` supports the `task-handler` lifecycle; its nodes correspond to the approved plan, implementation, refinement, typed verification record and guarded decision, documentation, selected-route review record and decision, goal assessment, the assessed outcome with its follow-up commitments, final user approval, and closeout. Its plan record has an optional `plan-handoff-artifact` item that an explicit task plan handoff fills.
