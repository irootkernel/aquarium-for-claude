---
name: design-qa
description: "Design or update durable local Design Gates with Ouroboros QA and an approved exact diff. Use when the user explicitly invokes /aquarium:design-qa."
...
After the approved envelope, use installed upstream `/ouroboros:interview` to resolve invariants and `/ouroboros:qa` to challenge every candidate gate. Require stable ID, concise title, invariant, scope, positive and failure scenarios, a local offline executable procedure that leaves the source repository unchanged and declares disposable outputs, objective pass condition, revalidation triggers, sources, and owner. Reject or defer any candidate that cannot meet all fields.
