...
After the approved envelope, use installed upstream `/ouroboros:interview`, `/ouroboros:pm`, or `/ouroboros:seed` only as needed. Produce one coherent epic with ordered work units and explicit ownership. Create or revise exactly one scope-local `TODO-*.md` dossier that declares the epic, owns its temporary goal, scope, task objectives, required and prohibited actions, and acceptance, is recorded in the adopted section of the TODO index, and is linked from the roadmap as `Detailed SOT`.
