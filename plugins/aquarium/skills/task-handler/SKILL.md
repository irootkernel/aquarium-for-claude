...
Otherwise read the Podway contract, use one `aquarium-task-v2` session for this canonical task after plan approval, coordinate with an explicitly authorized Claude Code todo list under the shared Podway contract, and record each verified phase handoff at the matching node. In `plan-only`, create neither goal nor session; in `plan-handoff`, create the session only after approval, create a Claude Code todo list only on explicit request, and stop at `implement` with the verified plan artifact. Do not let either goal mechanism or runtime evidence replace roadmap authority.
...
| Review | One exact complete task target received its required bounded Mulgae review; every actual requirement has a conservative criterion-level completion assessment; no criterion is `unmet` or `unverified`; every valid Medium-or-higher finding is fixed, checked, and re-reviewed; and every finding in the frozen eligible Low set has one completed shared disposition and current local verification. Historical Low counts may remain nonzero. |
...
- Before each review, derive the ordinal from operationally complete roots or verified composites for the goal revision, counting each original root once through the shared Mulgae contract. Such a run consumes it even on `request_changes` or failing CI; preflight, reads, and internal retry or extraction do not. Use Podway verbose exact IDs when active or an exact recoverable chain after opt-out; never use `latest`, objective inference, or an uncertain candidate. A session created from an earlier version of this managed Procedure is not migrated, and an unprovable ordinal stops. Exact recovery reruns and composition follow the shared contract; `followup` and `delta` cannot substitute for the next full-target review.
- In rounds one through three, supply `remediation-eligible` and stop immediately on approval. Reuse one Review Brief for preflight and execution, then aggregate every accepted selected-role Markdown report conservatively. Record native implementation and documentation finding counts as descriptive source facts, and record finding counts as consistent only when the unresolved total equals the Medium-or-higher plus Low buckets. Separately derive phase-owned obligation totals from CI remediation, valid findings, and unmet completion criteria; only those totals may select an owner. Route serially through finding confirmation, CI, completion, finding classification, explicit rework authority, then implementation, verification, and documentation ownership. Unverified completion returns to review evidence. CI failure, unmet completion, or Medium-or-higher findings enter the authority gate and then the first nonzero owner in that fixed phase order.
  ...
  - Handle Low findings as one frozen set identified by original root or direct-audit namespace, effective result, target, and source finding ID. Record the source basis, exact dispositions, the carried completion summary and zero unmet and unverified counts, pending and blocker counts, before and after targets, required local result, and honest coverage relationship. A Low disposition cannot erase or defer a completion gap. Advance through `decide-low-result` and `decide-low-completion` only when the local result passes, both Low counts are zero, and both carried completion-state counts are zero. `/aquarium:task-review` never fixes findings. Do not start another review or broad audit solely for Low or the accepted Low-only delta. An incomplete run enters supported recovery within its approved envelope or stops without a decision or blind retry. Resolve every `Needs confirmation` finding before selecting a review decision, using current read-only evidence or authorized checks. Otherwise leave the decision unset and ask for bounded confirmation authority. Reclassify it as `Valid` or `Invalid`, manually rework `review`, and reuse the ordinal unless the provider review runs again.
- Round four is confirmation-only, as is every user-authorized extra confirmation. Handle Low findings locally. When CI failure, unmet completion, or a Medium-or-higher finding establishes rework, stop at a fresh `choose-user-direction` attempt without selecting an outcome. Risk acceptance and deferral are unavailable. The user's `fix-and-review` decision authorizes only the resulting bounded correction and next-ordinal confirmation; its one transition through the phase-owner gates consumes that authority. If the confirmation still establishes rework, the new attempt must stop for a new user decision. Reset the ordinal only for an explicitly approved new goal revision.
...
Create a Claude Code todo list only after plan approval and an explicit user request under its tool contract. When authorized, inspect the current goal, continue it when it represents the same task, create one containing the task ID and evidence boundary when none exists, and stop rather than replace a different unfinished goal. Otherwise proceed without a Claude Code todo list. Omit a token budget unless the user explicitly supplied one.

When a Claude Code todo list is authorized, keep it active through every phase. Mark it complete only after `/aquarium:task-close` succeeds and no required task work or authorized lifecycle action remains. Mark the goal blocked only when the host's goal tool defines a blocked state and its own repeated-blocker rule is met by the same unresolved external blocker persisting across consecutive goal turns with no authorized action remaining; otherwise keep it active and report the exact gap.
