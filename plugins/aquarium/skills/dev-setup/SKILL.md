...
- For Dolgorae, recommend the newest verified official stable v0.1.x CLI at or above v0.1.1 while reporting release identity, executable checksum, platform, version envelope, and compatible capabilities independently. Dolgorae has no Aquarium paired skill or MCP registration, and this artifact's `/aquarium:independent-review` dispatches host subagents rather than routing through Dolgorae, so its admission is reported as third-party readiness rather than as a gate on Aquarium review.
...
When Mulgae or Gaori is selected, inspect its user-global and project-local MCP registrations independently. If neither is configured, offer `Configure global MCP` (recommended), `Configure project MCP`, and `Skip`. If a project-local registration exists in `.mcp.json`, ask whether it is intentional; preserve it only when the user confirms local scope. Otherwise propose removal of only that named local registration. Never infer local intent from the presence of `.mcp.json` or another local MCP entry.
...
- Humanizer's complete payload is its root `SKILL.md` plus `LICENSE`, installed at `humanizer` under the configured Claude Code skill root. Require `name: humanizer` and a v2 version matching the resolved tag.
- Before materializing im-not-ai's official payload, read the checked-out `install.sh` and require every write target to derive from an isolated temporary `CLAUDE_HOME`.
  Disclose that exact `install.sh --claude-only --copy` command and temporary target and obtain separate execution approval, then run it without `--force` and add the checkout's root LICENSE. Its Claude payload comes from the installer's own Claude source tree, not the one its other host mode materializes, and is three skills — `humanize-korean`, `humanize`, and `humanize-redo` — plus the four subagents the installer selects by default. Compare that complete regular-file tree with `<skill-name>` under the configured Claude Code skill root and with `<agent>.md` under the matching `agents/` directory, require `name: humanize-korean`, and never run the installer against the active configuration directory. The shipped inspector diagnoses only the `humanize-korean` skill of that payload; the other two skills and the subagents are established by this comparison alone.
...
Treat exact package installation or upgrade through `uv`, installing the Ouroboros Claude Code plugin — which is what provides the `/ouroboros:*` skills and the `plugin:ouroboros:ouroboros` MCP server — and the selected configuration action as separate persistent mutations. Display each command and its changed paths, obtain explicit approval, re-read targets immediately before mutation, and invalidate stale approval.

For full configuration, run only `ooo setup --runtime claude --non-interactive`. Full setup installs and prunes the packaged rules and upstream Ouroboros skills for the selected runtime and sets the runtime and LLM backend in `~/.ouroboros/config.yaml`; do not run `ooo setup refresh` first. It deliberately registers no MCP server, because that profile cannot share the isolated server process the plugin launches; omit `--mcp-mode`, which selects another host's MCP configuration mode. Offer `ooo setup refresh` as a separate repair alternative only when the user selects rules-and-skills refresh without full setup.

Approval for package mutation never authorizes plugin installation or configuration, and approval for one configuration path never authorizes the other. Setup and refresh must not call an Ouroboros provider, authenticate, run `auto`, `run`, `ralph`, or `evolve`, transmit repository source, create a Seed, or start an Aquarium design workflow.

After approved mutations, verify `ooo --version` and `claude mcp get plugin:ouroboros:ouroboros`. A resolving plugin-scoped name proves the plugin is installed and enabled and its skills are reachable; `Status: ✔ Connected` additionally proves the server is usable, and a resolved but unconnected server is a degraded registration rather than a missing one. Run `ooo mcp doctor --json` only for a hand-registered entry that directly launches the selected `ooo` executable; it inspects that package environment and is not evidence about the isolated server process the plugin launches.

When the active host can expose MCP tools safely, verify live exposure separately without invoking a provider.

Report missing plugin, registration, runtime configuration, and live exposure as distinct gaps, and tell the user when a Claude Code restart is required.
...
7. Apply only the approved shown diff. Then show the actual diff and verify the required structure, mandatory commit-message subsection, CLAUDE.md delegation including its `@AGENTS.md` import, repository-specific rules, and unrelated user content.
...
- Dolgorae supported release, official metadata verification, platform, version envelope, compatible capabilities, executable checksum, and identity stability separately;
