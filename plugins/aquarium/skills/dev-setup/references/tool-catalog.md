...
Diagnose four independent components with the v7 inspector's explicit `--include-ouroboros` flag: `ooo --version`, the plugin's registration, `ooo mcp doctor --json`, and the effective MCP registration. These probes are local and read-only: they do not contact a provider, initiate authentication, or make a network request, though the MCP doctor may inspect bounded local authentication-readiness metadata without exposing credential material. A healthy CLI does not prove that the plugin is installed, that the MCP runtime is ready, or that the server is reachable.

`ooo mcp doctor` reports the CLI's own environment, not the plugin's server. Because the supported layout keeps MCP 1.x in the CLI environment and runs the MCP 2 server in the isolated process the plugin launches, its `mcp_import` check and its exit code both fail on a correctly configured machine. Read the remaining checks for runtime health and treat a failing `mcp_import` alone as expected; never resolve it by adding MCP 2 to the CLI environment, which is the combination Ouroboros refuses.
...
Package installation, plugin installation, `ooo setup refresh`, and `ooo setup --runtime claude --non-interactive` are four separate persistent mutations with separate approvals. The refresh rewrites the rules, skills, bridges, and instruction guides a previous setup already installed for every detected runtime, without changing MCP registration, the runtime selection, or `~/.ouroboros/config.yaml`. Runtime setup writes the runtime and LLM backend into `~/.ouroboros/config.yaml` and deliberately registers no MCP server, because that profile cannot share the isolated server process the plugin launches; omit `--mcp-mode`, which selects another host's MCP configuration mode. The plugin is therefore the only supported source of the server: never hand-register a plain `ouroboros` entry to compensate. Re-read exact targets before each approved mutation and stop if they changed.

No setup action authorizes a provider call, authentication, repository-source transmission, `auto`, `run`, `ralph`, `evolve`, Seed creation, or an Aquarium design workflow. Verify only version, plugin registration, MCP doctor, effective registration, and, when safely observable in the active host, live MCP tool exposure. A Claude Code restart may be required after skill or MCP changes.
