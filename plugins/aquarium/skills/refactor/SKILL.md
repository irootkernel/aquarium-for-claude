...
After the approved envelope, use installed upstream `/ouroboros:interview` and `/ouroboros:seed` as needed. Produce one ordered epic whose work units keep compatibility, migration, rollback, and verification ownership explicit. Put an explicit `Design Gate impact` of `Not required`, `Pending`, or resolved `GATE-*` IDs on the epic and every implementation task, propagating the epic decision unless a task has a narrower resolved impact; new invariants remain pending for `/aquarium:design-qa`.

Run upstream `/ouroboros:qa`, adjudicate the draft, show the exact diff, and apply only after explicit approval and snapshot recheck. End with the epic identity, affected contracts, validation, gate status, and unresolved migration risks.
