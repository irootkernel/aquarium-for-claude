...
After all initial remediation groups complete, discard the prior matrix, findings, checks, and review result. Continue any authorized Claude Code todo list while repeating the direct audit and running exactly one round-two whole-epic Mulgae confirmation review from the latest committed snapshot. Do not start a third review automatically.
