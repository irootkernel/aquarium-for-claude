...
Before each Mulgae review, establish one Review Brief containing the actual requirement set, checkpoint, complete target, verification provenance, goal revision, ordinal, mode, roles, and approved scope. Use that same brief for preflight and execution through the shared Mulgae contract. Delegate native execution and recovery to `/use-mulgae`; use the shared prerequisite routing and consume only established terminal evidence. Aggregate every accepted selected-role Markdown report conservatively into the shared criterion-level completion assessment.
...
4. Accept a full-target root or its verified composite under the shared Mulgae contract, counting each original root once. Treat a Mulgae review as operationally complete under the shared contract, independently of CI outcome, finding adjudication, and semantic completion. Classify that complete review as clean only when `ci_decision=pass`, zero unresolved valid findings remain, and the completion assessment has zero `unmet` and zero `unverified` criteria; otherwise apply the bounded remediation or explicit disposition rules below. Zero findings, native role coverage, provider success, or exit status alone is insufficient.
...
Declare completion only when every required check has current passing evidence, whole-epic Mulgae evidence is operationally complete with passing CI, every actual requirement has a conservative completion assessment with zero `unmet` and zero `unverified` criteria, every member task and the epic have roadmap-defined successful states, no epic-owned residue remains, and one of these closeout conditions holds:
