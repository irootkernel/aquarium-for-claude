...
| `dev-setup` | Diagnose and configure repository-local development tools, and propose reference-based instruction-file guidance behind separate approvals. | `/aquarium:dev-setup` |
...
| `dev-setup-global` | Diagnose, install, and update the user-global CLIs, paired skills, services, and MCP registrations the repository skills depend on. | `/aquarium:dev-setup-global` |
...
| `mulgae-review` | Run one report-only standalone Mulgae review of an exact change target or a named task or epic completion candidate, then adjudicate its accepted role reports. | `/aquarium:mulgae-review` with a Mulgae-native `stage`, `workspace`, `dirty`, `diff`, or `patch` target and an optional task or epic ID |
...
The four design skills drive Ouroboros as a bounded leaf capability and need it installed and pinned to `>=0.51.1,<0.54.0`; `/aquarium:dev-setup-global` diagnoses and configures it behind separate approvals; the plugin itself is installed with `claude plugin`. They shape documents only and never implement.
...
The two static review skills share one contract, `references/review-contract.md`, which owns the source scopes, consent, static-review limits, and the result envelope, plus `references/review-intent-contract.md` for the Review Brief and the `change`-versus-`completion` purpose, `references/finding-disposition.md` for adjudication, and one target inspector and one repository-state inspector shipped inside `independent-review`. `orca-review` drives the Orca app rather than host subagents, so it also loads `references/orca-supervision.md` and needs the separately installed `orca-cli` Claude Code skill in the active catalog and a running Orca app; it loads Orca's orchestration guide through the Orca executable itself and stops, rather than approximating that contract, when the skill is absent. `independent-review` remains the subagent-based path, starts no Orca worker, uses no Dolgorae capture, and needs nothing extra; upstream v0.1.16 disabled its own Dolgorae-backed route and now sends a caller to an explicitly selected Orca or host-native subagent review, which is what this edition's skill has been since v0.1.14, so it stays enabled here. Because neither backend holds an immutable capture, both support the same four scopes — `staged`, `head`, `commit`, and `range` — and upstream's capture-only `workspace` and `dirty` scopes are unsupported. `mulgae-review` is the third standalone entry point: it builds the same Review Brief for Mulgae's own native targets and consumes the accepted role reports under `references/mulgae-review-contract.md`, while the separately installed `/use-mulgae` skill owns the native lifecycle.
...
`sync.py` copies the upstream plugin, drops the files excluded by name, applies literal substitutions, applies overrides, derives invocation gating from the upstream sidecars and drops them, then adds the host-only files — the bundled reviewer subagent, the edition-owned `upgrade` skill, which must declare its own gating or generation refuses it, and the repository-state inspector `independent-review` runs. It refuses to run against an empty submodule, refuses to run when upstream grows a plugin-root directory or file the transformation does not handle, fails if host-specific text survives, fails if a substitution rule matched nothing that ships, and fails if a generated script cannot run.

Six files diverge semantically and are kept as overrides rather than substitutions:

| Override | Why |
|---|---|
| `references/review-contract.md` | v0.1.14 rewrote the shared contract around upstream's new backend split and bound independent review to a Dolgorae immutable capture running a fresh Codex Reviewer. Neither half is true here, and the divergence runs through the scope table, backend ownership, settlement, and the result envelope rather than through one clause. The override keeps upstream's scope meanings, consent, static limits, and disposition rules, marks the two capture-only scopes unsupported for the same reason they are unsupported for Orca Review, and replaces the capture with a repository-state baseline that proves nobody mutated the repository during the review rather than that the target was immutable. v0.1.16 carries the Review Brief, the `change`-versus-`completion` purpose, proportional change inspection, and Orca's operational-deviation policy; the override adopts all four, applies the proportional rule to both backends, and keeps the subagent backend where upstream now records a disabled route. |
| `skills/independent-review/SKILL.md` | Replaces upstream's Dolgorae-and-Codex backend with fresh read-only subagents dispatched through the host's own mechanism, preferring the bundled `aquarium:independent-reviewer`. A reviewer runs under the same provider as the coordinator, so the skill claims a fresh context rather than an independent provider, keeps depth on Opus and breadth on Sonnet, and buys coverage by giving several reviewers distinct lenses. The shared contract is written for one out-of-process reviewer, so the override also maps its singular reviewer, verdict, and backend-lifecycle fields onto one row per dispatched subagent. Upstream's v0.1.16 file is a refusal stub for its disabled Dolgorae route, so the override is now the whole skill: it carries the Review Brief and purpose into every reviewer specification, applies upstream's proportional-review and actionable-finding rules to the reviewers, and has the coordinator issue the final technical verdict after the repository-state comparison. |
| `skills/dev-setup-global/SKILL.md` | v0.1.15 split user-global installation out of `dev-setup`, and this edition's global divergence moved with it. Paired and third-party skills install under the configured Claude Code skill root, im-not-ai materializes its Claude payload, and the Ouroboros component shrinks to the one installation this host has: it arrives as a plugin rather than as per-home rules, skills, and an MCP registration, so global setup reports its CLI version and plugin-scoped registration and leaves installing or upgrading it to `claude plugin`. The bundled `aquarium-dev` channel is not shipped here, so it is not offered either. v0.1.16 routes plugin installation and updates to the host's own plugin management, which here is `claude plugin`. |
| `skills/dev-setup/references/agents-guidance.md` | Same structure as upstream, now its seven core behaviors. Claude Code does not read `AGENTS.md` on its own but does resolve a `@AGENTS.md` import before the first turn, so the delegation file carries the import rather than a request to go read the file, and diagnosis reports prose-only delegation as a gap. |
| `references/tool-catalog.md` | Registers Mulgae and Gaori MCP with `claude mcp add -s user`, keeps `.mcp.json` as the explicit project override, and verifies the user, project, and effective views from the configuration files rather than from `.codex/config.toml` and typed `codex mcp get --json` output. v0.1.15 moved it out of `dev-setup` into the shared `references/`, dropped the `aquarium-dev` section with the excluded channel, and replaced upstream's per-home Ouroboros machinery with this host's plugin integration. v0.1.16 raised the Sanho, Mulgae, Gaori, and Sorage floors and added a `codex mcp list --json` inventory as absence proof; the floors are ported and the inventory is not, because `claude mcp list` health-checks every approved server and so starts it, and the sentence about the excluded development channel's producer manifests is dropped with the channel. |
| `references/review-intent-contract.md` | New in v0.1.16 as the shared Review Brief and `change`-versus-`completion` contract, with two sections written for upstream's disabled route: a routing matrix that sends an `independent-review` caller to an explicitly preselected Orca or "native Codex" subagent review, and a description of that native route. Here `independent-review` is the native subagent route and stays enabled, so the override keeps the Review Brief, provenance, candidate-boundary, and assessment sections verbatim and rewrites only those two sections and the contract's opening sentence around the backend this edition has. |

`skills/dev-setup/SKILL.md` was an override until v0.1.15 moved user-global installation into `dev-setup-global`, and `skills/dev-setup-global/SKILL.md` took its place. What it still diverges on is three literal spans, two of which carry a forbidden needle and would need a rule whether or not the override stayed, so the override was retired and three substitutions took its place — the counted swaps reproduce the retired override byte for byte.

Everything else is a literal substitution: the `$aquarium:` sigil becomes `/aquarium:`, the `$use-*` skill sigils become `/use-*`, the `$create-podway-procedure` maintainer-skill sigil becomes `/create-podway-procedure`, the Ouroboros sigils `$interview`, `$pm`, `$seed`, and `$qa` become `/ouroboros:*` because Ouroboros installs as a Claude Code plugin rather than user-scoped skills, the separately installed `$deslop` becomes `/deslop`, `request_user_input` becomes `AskUserQuestion` — and task-close's "when available" hedge on it is dropped, because the tool is always present on this host, Lora installs with `--agent claude-code`, the `Codex goal` a workflow coordinates with becomes the Claude Code todo list, the inspection script resolves skills from the Claude Code roots alone — `CLAUDE_CONFIG_DIR` and `~/.claude/skills` — and diagnoses Ouroboros against this host instead of Codex, user-scoped skills install into `~/.claude/skills`, `release-qa`'s host-neutral "available agent delegation surface" becomes this host's own subagent mechanism with parallel clusters launched in a single message — the mechanism rather than the tool's name, because Claude Code has already renamed that once — `orca-review`'s refusal of the two capture-only scopes says that independent review does not support them either rather than that it is disabled, the shared disposition contract names independent review as this host's own subagent route and stops grouping it with the backends that capture, the writing-skill inspection expects Humanizer and im-not-ai in the Claude Code skill root rather than the shared cross-agent root and the Codex home, the presence-only trust table v0.1.15 added for the paired and third-party skills resolves through that same root, and the hook command resolves `${CLAUDE_PLUGIN_ROOT}` instead of Codex's `${PLUGIN_ROOT}`.
...
A literal substitution rule fails just as quietly in the other direction. v0.1.10 dropped one Oxford comma from `Ouroboros package, Codex, and runtime components` and the rule stopped matching without a word, and three script rules died in the same release when upstream extracted a shared helper and added a parameter. Three script rules died again in v0.1.13, when upstream extracted the Ouroboros transport comparison into two launcher matchers and split the runtime component across the installed and not-installed paths, and were re-derived against the new anchors. Every rule must now rewrite text that ships: generation counts matches outside the override targets and fails naming any rule that matched nothing, so a dead rule is deleted or re-derived deliberately instead of rotting. The phrases that only ever occurred inside an override target were deleted for the same reason; the forbidden needles and the sigil scan still catch that text if an override is ever retired. Eight script rules died at once in v0.1.16: six because that probe-shape change reached every function that read the old probe, and two because upstream hard-coded the shared cross-agent skill root in `inspect_im_not_ai` and the trust table. The two Ouroboros registration rules became one whole-function rule and the rest were re-anchored.
...
v0.1.14 added the second file exclusion. Upstream routed independent review through a Dolgorae immutable capture that runs a Codex Reviewer, and shipped `references/dolgorae-review-contract.md` to define that backend's candidate, capture, and settlement rules. This artifact dispatches host subagents instead, and its `orca-review` is forbidden to use Dolgorae, so nothing here routes through that contract and it would document a lifecycle the edition does not have. Dolgorae itself stays in the tool catalog as a selectable third-party CLI; only the consumer contract is dropped, and the same reference check keeps the two review overrides from naming it.

v0.1.15 added two more, both from the development channel upstream bundled that release. `references/development-contract.md` is the shared contract between the `aquarium-dev` producers and their host manager, and `skills/dev-setup-global/scripts/inspect_ouroboros.py` measures per-Codex-home Ouroboros rules, skills, and MCP registration by importing `ouroboros.codex.artifacts` and comparing each discovered home. Neither describes anything this artifact has, and the module would have shipped an import of a package this edition never installs.

The channel itself is excluded at the plugin root rather than by file. v0.1.15 bundled a Darwin arm64 CLI and stdio MCP runtime under `tools/aquarium-dev/` — it builds unreleased local-main artifacts of Aquarium and its producer CLIs under `~/.aquarium-dev`, passes `CODEX_HOME` through to them, and produces Codex plugin artifacts — and registered it from a plugin-root `.mcp.json` written in the other host's `mcp_servers` shape. No skill here routes through it, its 37 KB hash-pinned wheel lock would ship dead weight, and a translated registration would start a server no installation provides and report a failed MCP connection in every session of every repository.

That exclusion needed a guard of its own. `COPIED_DIRECTORIES` gained a counterpart check when `hooks/` was dropped in silence, but the check filtered on directories, and nothing copies a plugin-root file at all — so `.mcp.json` would have vanished exactly the same way. Generation now aborts on an unhandled plugin-root file as well as an unhandled directory, each plugin-root exclusion must still exist upstream, and `tests/validate.rb` reads that list out of `scripts/sync.py` so an exclusion retired there stops being tolerated here in the same commit.

The `Codex` name is otherwise forbidden in generated text. One file is exempt: `tool-catalog.md` names the Codex CLI as a Mulgae review provider — provider opt-in, the required CLI version, the native login, several credential identities, and the executable — which stays true here. The exemption records the upstream digest it was judged against, so the sync stops when that file changes. v0.1.11 rewrote `orca-review/references/provider-contracts.md` around named providers and left no `Codex` mention in it, so that exemption was retired rather than rotated, and v0.1.15 retired the two `aquarium-dev` exemptions with the skill upstream deleted. Each rotation re-reads every mention the override ships before the digest moves.

The Mulgae and Gaori MCP probes are re-targeted for the same reason. Upstream reads them through `codex mcp get --json`, with `CODEX_HOME` pointed at `.codex/` for the local view, so on this host the inspector could never see the registration its own catalog tells the user to create, and `--require-mulgae-mcp` reported the tool degraded forever. The generated inspector reads Claude Code's three views from configuration alone — the user-scope entry and the private per-project entry from `.claude.json`, honouring `CLAUDE_CONFIG_DIR`, and the shared entry from `.mcp.json` with its approval state — and derives the effective view from the documented precedence. It deliberately never calls `claude mcp get`, because that health-checks an approved server and so starts it, and setup must not start the server. A `.mcp.json` server that nobody has approved yet is `unverifiable` with `registration_pending_approval`, not degraded. Two whole functions are replaced so the anchors are the most stable text upstream has, and `tests/test_claude_mcp_inspection.py` exercises the shipped bytes against configuration fixtures under a private `CLAUDE_CONFIG_DIR`. v0.1.16 moved the neutral-cwd user-scope probe out of the global inspector into a new `inspect_global_mcp_scope` in the project inspector, so the rule moved with it: that function now returns the configuration read's user-scope view, and the global inspector's one-line delegation ships as upstream wrote it.

One forbidden needle grew a tilde too many. `~/.agents/skills` catches the shared cross-agent root in prose, but Python writes it as `Path.home() / ".agents/skills"`, which never matches — and v0.1.15 wrote thirteen more of those — eleven in a new presence-only trust table, one in the Lore readiness comparison, and one in Deslop's — where v0.1.14 had two in all. Left alone, an inspector that searches only the Claude Code roots would have compared every discovered installation against a path those roots can never produce and reported correct installations degraded forever, which is the same failure the writing skills had in v0.1.14. The needle is now `.agents/skills` in both lists, so the Python spelling aborts generation instead of shipping.
